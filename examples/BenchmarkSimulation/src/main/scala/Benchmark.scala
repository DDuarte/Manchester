...
class Benchmark(val labelWidth: Int = 0) {
  ...
  case class Tms(ctime: Long = 0, utime: Long = 0, real: Long = 0) {
    ...
    /**
      * Times as a String.
      */
    override def toString = "%.6f, %.6f, %.6f, %.6f".format(s("user"), s("system"), s("total"), s("real"))
  ...
  /**
    ...
    */
  private lazy val header = println("desc, user, system, total, real")
  ...
  def measure(code: => Unit) = {
    ...
    /* val runs: collection.mutable.ListBuffer[Tms] = collection.mutable.ListBuffer()

    for (i <- 0 to 10) {
      val startTime = Tms()
      code
      runs += startTime.elapsed
    }

    runs.foldLeft(new Tms())( _ + _ ) / runs.length */
