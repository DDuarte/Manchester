...
class Page(val id: String, val links: MSet[Page], val tags: Set[String], val product: Option[Product]) {
...
object Page {
  def apply(id: String, links: MSet[Page], tags: Set[String], product: Option[Product]) = {
...
class WebsiteState {
  ...
  override def toString: String = {
    ...
    purchases.toSeq.sortWith(_._2 > _._2).foreach {
      case (product, count) =>
        sb ++= f"${product.name}%50s $count%5d ${decFormat.format(count * product.price)}%10s${product.currency}\n"
    }
      ...
      ("name" -> "Simulation") ~
        ...
        ("visits" ->
          ...
          ("purchases" ->
            ...
            ("userAgentName" -> sim.userFactory
              .getClass.getGenericInterfaces.head
